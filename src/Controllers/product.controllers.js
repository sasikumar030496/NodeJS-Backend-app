...
exports.createProduct = async (req, res) => {
  ...
  if (!price || price < 0) {
    return res
      .status(400)
      .send({ message: "Price cannot be null or negative" });
  }
  if (!name) {
    return res
      .status(400)
      .send({ message: "Name of the product can't be null" });
  }
  const productData = {
    name,
    description,
...
exports.updateProduct = async (req, res) => {
  ...
  try {
    const product = await Product.findByIdAndUpdate(
      ...
      { new: true }
    ); // Mongoose method to find object by ID and update details from req.body, new:true is used to display updated details while calling API
    if (!product) {
      return res.status(400).send({ message: "Product not found" });
    }
    return res.status(200).send(product);
